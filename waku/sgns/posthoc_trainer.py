...
from waku.word_similarity.base import load_sim_data, evaluate
from waku.utils.metrics import measure_sparsity
...
class Word2Vec:
    ...
    def train(self, 
        ...
        posthoc_epoch : `int`
            Save embeddings and test on WordSim-353 every posthoc_epoch.
        wordsim353_path : `str`            
            Filepath for the WordSim-353 dataset.
        ...
        # Load WordSim-353 data
        wordsim353_data = load_sim_data(wordsim353_path, 'WordSim353')        
        stats = {"wordsim353_score":list(), 
                 "mean":list(),
                 "std":list(),
                 "n_zero_vecs":list()}
        prune_thresholds = np.append(np.array([0]), np.logspace(-5,1,100)).tolist()
        ...
        for epoch in range(epochs):
            ...
            if posthoc_epoch !=-1 and epoch % posthoc_epoch == 0:
                # Get embedding
                embedding = copy.deepcopy(self.get_embeddings())
                # Save mean and standard deviation of embedding matrix
                stats["mean"].append(embedding.mean())
                stats["std"].append(embedding.std())
                # Prune embeddings to 90%
                for prune_thresh in prune_thresholds:
                    embedding[np.absolute(embedding) < prune_thresh] =  0
                    if measure_sparsity(embedding) >=0.88:
                        # Test of WordSim353 and Save
                        sp_rank_coeff, sp_rho = evaluate(embedding,
                                                         wordsim353_data, 
                                                         self.word2index,
                                                         verbose=False)
                        print("WordSim353 score: ", sp_rank_coeff)
                        stats["wordsim353_score"].append(sp_rank_coeff) 
                        # Save numer of zero row vectors of embedding matrix
                        stats["n_zero_vecs"].append(np.sum(~embedding.any(1)))
                        break
